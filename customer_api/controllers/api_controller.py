...
class CartAPIController(http.Controller):
    ...
    def _authenticate_session(self):
        ...
        try:
            ...
            if not hasattr(request, 'session') or not request.session:
                _logger.error("No session found")
                return False

            # Check if user is authenticated (not public user)
            public_user_id = request.env.ref('base.public_user').id
            if not request.uid or request.uid == public_user_id:
                _logger.error(f"User not authenticated. UID: {request.uid}, Public UID: {public_user_id}")
                return False
            ...
            try:
                ...
                if not user.exists() or not user.active:
                    _logger.error(f"User {request.uid} does not exist or is inactive")
                    return False

                _logger.info(f"User authenticated successfully: {user.name} (ID: {user.id})")
                return True

            except Exception as e:
                _logger.error(f"Error checking user: {e}")
                return False

        except Exception as e:
            _logger.error(f"Authentication check failed: {e}")
            return False
    ...
    def _get_or_create_cart(self, partner_id):
        ...
        try:
            ...
            if not cart:
                ...
                _logger.info(f"Created new cart {cart.id} for partner {partner_id}")
            else:
                _logger.info(f"Using existing cart {cart.id} for partner {partner_id}")

            return cart

        except Exception as e:
            _logger.error(f"Error getting/creating cart: {e}")
            raise

    @http.route('/api/cart/add', type='json', auth='user', methods=['POST'], csrf=False)
    def add_to_cart(self, **kw):
        ...
        try:
            ...
            _logger.info(f"Add to cart request data: {data}")
            ...
            try:
                ...
                _logger.error(f"Invalid data format: {e}")
            ...
            _logger.info(f"Adding product {product_id} to cart for user {user.name} (partner {partner.id})")
            ...
            if not product.sale_ok or not product.active:
                return {'status': 'error', 'message': 'Product not available for sale'}
            ...
            if existing_line:
                # Update quantity
                old_qty = existing_line[0].product_uom_qty
                existing_line[0].product_uom_qty += quantity
                _logger.info(f"Updated existing line: {old_qty} -> {existing_line[0].product_uom_qty}")
            else:
                # Add new line
                line_vals = {
                    'order_id': cart.id,
                ...
                _logger.info(f"Created new cart line {new_line.id}")

            # In Odoo 17, amounts are computed automatically - no manual trigger needed
            _logger.info(f"Cart totals will be computed automatically")
            ...
            _logger.error(f"Error adding to cart: {str(e)}", exc_info=True)
            return {'status': 'error', 'message': f'Failed to add product to cart: {str(e)}'}

    @http.route('/api/cart/view', type='json', auth='user', methods=['POST'], csrf=False)
